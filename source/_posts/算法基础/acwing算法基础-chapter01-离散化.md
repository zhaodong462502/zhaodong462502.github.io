...
***all集合已经是按x的值排好序的集合，find函数只是为了找到x在all集合里位置。***

> 2023-10-04 重新打卡时，在all集合处理耗时过长，总想把all集合里元素通过find函数离散化，其实all集合在排序去重之后已经是离散化好的状态，只需对add集合处理即可。
...
要考虑到add集合会在相同位置多次添加元素

```
a[pos]+= c;  #写成a[pos] = c;会漏掉此种情况
```
